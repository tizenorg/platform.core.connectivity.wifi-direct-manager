...
static int _ws_query_generation(unsigned char* MAC, wfd_oem_service_e type, char *data, char *buff)
{
	...
	if(data && (tlv_len = strlen(data)))
	{
		...
		{
			snprintf(buff, CMD_LENGTH, WS_CMD_P2P_SERV_DISC_REQ MACSTR " upnp %s", MAC2STR(MAC), data);
		}else{
			...
			snprintf(buff, CMD_LENGTH, WS_CMD_P2P_SERV_DISC_REQ MACSTR " %s%s", MAC2STR(MAC), query, data);
		}
	}else{
		snprintf(buff, CMD_LENGTH, WS_CMD_P2P_SERV_DISC_REQ MACSTR " %s", MAC2STR(MAC), query);
	}
	if(query != NULL)
		free(query);
	__WDP_LOG_FUNC_EXIT__;
