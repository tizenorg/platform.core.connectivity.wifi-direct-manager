...
static int _ws_query_generation(char *buff, int size, unsigned char* MAC, int type, char *data)
{
	__WDP_LOG_FUNC_ENTER__;
	int res=0;
	char *query=NULL;
	...
	if(data && strlen(data))
	{
		if(type == 2)
		{
			snprintf(buff, size, WS_CMD_P2P_SERV_DISC_REQ MACSTR "upnp %s", MAC2STR(MAC), data);
		}else{
			query[0] = '0' + (char)(strlen(data)/16);
			query[1] = '0' + strlen(data)%16;
			snprintf(buff, size, WS_CMD_P2P_SERV_DISC_REQ MACSTR "%s%s", MAC2STR(MAC), query, data);
		}
	}else{
		snprintf(buff, size, WS_CMD_P2P_SERV_DISC_REQ MACSTR "%s", MAC2STR(MAC), query);
	}
	if(query != NULL)
		free(query);
	__WDP_LOG_FUNC_EXIT__;
...
int ws_serv_disc_req(unsigned char* MAC, int service_type, char *data)
{
	...
	res = _ws_query_generation(cmd, sizeof(cmd), MAC, service_type, data);
	if (res < 0) {
...
int ws_serv_disc_cancel(int identifier)
{
	...
	snprintf(cmd, sizeof(cmd), WS_CMD_P2P_SERV_DISC_CANCEL " %0xllx", identifier);
