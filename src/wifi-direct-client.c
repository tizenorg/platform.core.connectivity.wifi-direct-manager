...
char *wfd_server_print_cmd(wifi_direct_cmd_e cmd)
{
	...
	case WIFI_DIRECT_CMD_ENABLE_WIFI_DISPLAY:
		return "WIFI_DIRECT_CMD_ENABLE_WIFI_DISPLAY";
									...
									gpointer user_data)
{
	...
	case WIFI_DIRECT_CMD_ENABLE_WIFI_DISPLAY:
		{
			res = wfd_manager_enable_wifi_display(req.data.int1);
			if (res < 0) {
				WDS_LOGE("Failed to enable or disable wifi display");
				rsp.result = WIFI_DIRECT_ERROR_OPERATION_FAILED;
			}
		}
		break;
	case WIFI_DIRECT_CMD_SET_DISPLAY:
		{
			...
			res = wfd_local_set_display(req.data.int1, req.data.int2, hdcp);
			if (res < 0) {
