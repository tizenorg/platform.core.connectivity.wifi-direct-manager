									...
									gpointer user_data)
{
	...
	case WIFI_DIRECT_CMD_SERVICE_ADD:
		...
		break;
	case WIFI_DIRECT_CMD_SERVICE_DEL:	// session
		{
			char *buff = NULL;
		...
		break;
	case WIFI_DIRECT_CMD_SERV_DISC_REQ:	// session
		{
			char *buff = NULL;
			...
			rsp.param1 = manager->query_handle_cnt;

			if(buff != NULL)
				free(buff);
		}
		break;
	case WIFI_DIRECT_CMD_SERV_DISC_CANCEL:	// session
		{
			int handle = req.data.int1;

			res = wfd_manager_serv_disc_cancel(manager, handle);
			if (res < 0) {
